...
export default function ContactsPanel({ open, onClose }: ContactsPanelProps): React.ReactElement | null {
  ...
  const fetchLists = async (force = false) => {
    ...
    setError("");
    try {
      const lists = await fetchSendGridLists(force);
      setLists(lists);
    } catch (err) {
  ...
  const loadContacts = async (list: SGList) => {
    ...
    try {
      const contacts = await fetchSendGridContacts(list.id, {
        // Show "Exporting 4737 contacts... (12s)" with the list's known count.
        onProgress: (msg) => setLoadingMsg(msg.replace("contacts...", `${list.count} contacts...`)),
      });
    ...
    <div className="fixed top-0 right-0 bottom-0 w-[520px] bg-[var(--bg-card)] shadow-[var(--shadow-panel)] z-[1000] flex flex-col panel-slide">
      ...
      <div className="flex justify-between items-center px-5 py-4" style={{ background: BRAND.navy }}>
        ...
        <div className="flex gap-2">
          {view === "lists" && (
            <button
              onClick={() => { invalidateSendGridListsCache(); fetchLists(true); }}
              disabled={listsLoading}
              title="Refresh from SendGrid (lists are cached for 5 min)"
              className="text-[10px] font-bold text-[var(--color-sky)] bg-transparent border border-[var(--color-sky)] rounded px-2 py-1 cursor-pointer disabled:opacity-50">
              {listsLoading ? "..." : "\u21BB Refresh"}
            </button>
          )}
          {view !== "lists" && (
          ...
          <>
            ...
            {!listsLoading && error && (
              <div className="mb-3 p-3 rounded-md border border-red-300 bg-red-50">
                ...
                <button onClick={() => fetchLists(true)} className="mt-2 text-[10px] font-bold text-red-600 bg-white border border-red-300 rounded px-2 py-1 cursor-pointer">Retry</button>
              </div>
            )}
